...
type Iterator = { like: number; comment: number; vote: number };
type LikeResult = { likedFlag: boolean; liked?: number; missing?: number };
...
export class VnExpressService {
    ...
    async likeVnex(
        { url, browserNum, likeLimit, isVisual, proxy: resetProxy, continueChunk }: VnExpressLikeQuery,
        body: string,
    ) {
        ...
        let i = 0;
        let data: Iterator = { like: 0, comment: 0, vote: 0 };
        ...
        for (; i < accountChunk.length; i++) {
            ...
            for (let j = 0; j < vnExComments.length; j++) {
                ...
                for (const result of resultChunk) {
                    ...
                    else results[j].liked = result.results[j].liked;
            ...
            this._logger.service.log(resultChunk.map(result => JSON.stringify(result)).join('\n'));
    ...
    private async _fetchVnExComments(url: string, body: string) {
        ...
        const dataMap = new Map(data.map(item => [item.content.trim(), item]));
        const { commentNotFound, vnExComments } = comments.reduce(
            (acc, comment) => {
                const item = dataMap.get(comment);
                if (!item) acc.commentNotFound.push(comment);
                else acc.vnExComments.push(item);
                return acc;
            },
            { commentNotFound: [] as string[], vnExComments: [] as VNExDataItem[] },
        );
        ...
        if (!fs.existsSync(cookiesPath)) {
            ...
            return await retry(
                async () => {
                    try {
                        await ppt.start();
                        await ppt.run(this.__redirect(id, url, 'section.section.page-detail.middle-detail'));
                        await ppt.run(this.__autoScroll(id));
                        await ppt.run(this.__login(id, profile, EMethod.LIKE));
                        await sleep();
                        await ppt.run(this.__autoScroll(id));
                        await ppt.run(this.__loadmoreComments(id));
                        return await ppt.run(this.__like(id, comments, likeLimit));
                    } catch (error) {
    ...
    private __redirect(id: string, url: string, selector: string) {
        ...
        return async function (page: PageWithCursor) {
            ...
            await Promise.race([page.goto(url, { waitUntil: 'networkidle2' }), page.waitForSelector(selector)]).catch(
                _ => {},
            );
        };
    }

    private __autoScroll(id: string, maxScrolls = 10) {
        const log = this._logger.instance(id).log;
        ...
        return async function (page: PageWithCursor) {
            ...
            while (true) {
                log(`Đang chạy auto scroll #${scrolls}...`);
                const shouldBreak = await page.evaluate(() => {
                    // Random distance from 300 - 1200px
                    const distance = Math.floor(Math.random() * 1200 + 300);
    ...
    private __like(id: string, comments: VNExDataItem[], likeLimit?: number) {
        ...
        return async function (page: PageWithCursor) {
            for (const { comment_id, userlike, content } of comments) {
                // Scroll comment into view and throw error if comment not loaded in DOM
                const commentExist = await page.evaluate(comment_id => {
                    const el = document.querySelector(`a[id="${comment_id}"]`);
                    if (!el) return false;
                ...
                if (!commentExist) {
                    error(`Comment ${comment_id} không tồn tại (DOM not loaded)`);
                    results.push({ likedFlag: false });
                    await sleep();
                    continue;
                }
                ...
                const liked = currentLike - userlike; // Số lượng like tăng từ khi chạy chương trình
                ...
                const missing = likeLimit - liked; // Số lượng like còn thiếu
                ...
                try {
                    await Promise.all([
                        ...
                        page.waitForResponse(
                            r => r.url() === 'https://usi-saas.vnexpress.net/api/cf' && r.status() === 200,
                        ),
                    ]);
                    results.push({ likedFlag: true, liked, missing });
                    ...
                    // Assuming reading 4 words take 1s => n words take n/4 secs. We gotta wait (n/4) + (1-2) secs
                    await sleep((contentLength / 5) * 1000 + Math.floor(Math.random() * 2000 + 1000));
                }
            }
